...
`go-ruby-mustache/mustache` reimplements Ruby's `mustache` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-mustache/mustache` directly, with no Ruby runtime;
